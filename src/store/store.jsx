...
const initialState = {
  ...
  journal: {
    entries: [
      { id: 1, date: 'Nov 12, 2025', text: 'Today was a good day. I felt really productive and managed to complete most of my tasks.' },
      { id: 2, date: 'Nov 11, 2025', text: 'Had a rough morning but turned it around by going for a walk in the afternoon.' },
    ],
    currentText: '',
    sortBy: 'Entry Date',
    showMenu: false,
  ...
  switch (action.type) {
    ...
    case 'ADD_TASK':
      return {
        ...
        planner: {
          ...state.planner,
          tasks: [...state.planner.tasks, { id: Date.now(), title: action.title, duration: action.duration || null, done: false }],
          showAddTask: false,
        },
      }
    case 'DELETE_TASK':
    ...
    case 'PLANNER_PREV_MONTH': {
      ...
      if (m < 0) { m = 11; y-- }
      return { ...state, planner: { ...state.planner, currentMonth: m, currentYear: y } }
    }
    ...
    case 'PLANNER_SELECT_DATE':
      return { ...state, planner: { ...state.planner, selectedDate: action.date } }
    case 'PLANNER_SHOW_MENU':
      return { ...state, planner: { ...state.planner, showMenu: action.val } }
    case 'PLANNER_SHOW_ADD':
      return { ...state, planner: { ...state.planner, showAddTask: action.val } }
    case 'PLANNER_SET_SORT':
      return { ...state, planner: { ...state.planner, sortBy: action.val, showMenu: false } }
    ...
    case 'TOGGLE_SAVER': {
      ...
      const allDone = savers.every(s => s.done)
      return { ...state, focus: { ...state.focus, savers, showCompletion: allDone } }
    }
    case 'FOCUS_CLOSE_COMPLETION':
      return { ...state, focus: { ...state.focus, showCompletion: false } }

    // JOURNAL
    case 'JOURNAL_SET_TEXT':
      return { ...state, journal: { ...state.journal, currentText: action.text } }
    case 'JOURNAL_SAVE': {
    ...
    case 'JOURNAL_SHOW_MENU':
      return { ...state, journal: { ...state.journal, showMenu: action.val } }
    case 'JOURNAL_SET_SORT':
      return { ...state, journal: { ...state.journal, sortBy: action.val, showMenu: false } }
    case 'JOURNAL_TOGGLE_LOCK':
      return { ...state, journal: { ...state.journal, locked: !state.journal.locked, showMenu: false } }
